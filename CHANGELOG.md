...
- ✅ 宠物属性会随时间自然衰减
- ✅ 每天不互动：饥饿值 +15，快乐值 -10
- ✅ 最大累计衰减：饥饿值 +50，快乐值 -50
- ✅ 基于 `last_care_at` 时间戳自动计算
- ✅ 页面加载时自动应用衰减
...
- ✅ 每种宠物拥有三种表情状态
  ...
  - 饥饿表情：饱食度 < 30（张大嘴巴）
  - 伤心表情：快乐值 < 30（眉毛下垂）
- ✅ 10 种宠物 × 3 种表情 = 30 个独特 SVG 设计
- ✅ 根据当前数值动态切换表情
- ✅ 页面显示心情提示文本

**技术实现** (app.py)
- `calculate_pet_decay()` 函数 (806-825)：计算基于天数的衰减量
- `get_pet_mood()` 函数 (828-841)：根据数值判断心情状态
- `pet()` 路由更新 (1650-1714)：
  - 加载页面时应用衰减并保存到数据库
  - 根据心情选择对应的 SVG 表情
  - 传递心情状态到模板
...
**数据结构更新**
- `PET_APPEARANCES` 配置 (348-429)
  - 从单一 `svg` 字段改为三个字段
  - `normal_svg`：正常开心的表情
  - `sad_svg`：伤心的表情（眉毛下垂、嘴巴下弯）
  - `hungry_svg`：饥饿的表情（眼睛放大、嘴巴张开）

**界面优化**
- 宠物页面新增心情提示 (templates/pet.html:9-13)
  - 饥饿时显示红色警告："⚠️ XX饿了，快喂点东西吧！"
  - 伤心时显示灰色提示："😢 XX有点不开心，多陪陪它吧！"
- 选择页面更新使用 `normal_svg` (templates/choose_pet.html:20)

**游戏平衡**
- 每日衰减速度：中等（需要约 3 天不照顾才会触发低状态）
- 饥饿阈值：30（饱食度低于 30 显示饥饿表情）
- 快乐阈值：30（快乐值低于 30 显示伤心表情）
- 优先级：饥饿状态 > 伤心状态（更紧急）

---

## v2.5.1 - 管理员宠物状态管理 (2025-11-09)

### 👨‍💼 管理员新增功能

**宠物状态调节权限**
- ✅ 管理员可调节任意用户的宠物饥饿值和快乐值
- ✅ 数值范围：0-100，自动限制在有效范围内
- ✅ 调整后自动更新 `last_care_at` 时间戳
- ✅ 显示饱食度（100 - 饥饿值）和快乐值反馈

**用户管理表格更新**
- ✅ 新增"宠物状态"列 (templates/admin.html:196)
- ✅ 内联表单显示当前饥饿值和快乐值
- ✅ 输入框支持 0-100 的数值调整
- ✅ 自动填充用户当前宠物状态（无宠物时默认 50/60）

**技术实现**
- 新增路由：`admin_set_pet_stats()` (app.py:1503-1557)
  - 接收饥饿值和快乐值参数
  - 使用 `clamp()` 函数限制数值范围
  - 支持更新已有宠物或创建新宠物
  - 更新后返回友好的状态反馈
- 管理员查询更新 (app.py:1185-1192)
  - 添加 `pets.hunger` 和 `pets.happiness` 字段
  - 通过 LEFT JOIN 获取宠物状态
...
## v2.6.0 - 住宿生验证码系统 (2025-11-09)

### 🎓 住宿生特权系统

**系统背景**
- 面向6-8年级住宿生，解决在校期间无法照顾宠物的问题
- 学生周一到周五在校提交纸质版读后感给老师
- 老师在周五放学时发放验证码作为奖励
- 学生回家后在网站兑换验证码
...
**用户界面**
- 新增"住宿生兑换"导航入口 (templates/base.html:20)
- 兑换页面 (templates/redeem_code.html)
  - 验证码输入框（8位，自动转大写，等宽字体）
  - 住宿生状态显示（激活/未激活）
  - 有效期倒计时
  - 兑换历史记录（最近10条）
  - 使用说明和温馨提示
...
**管理员界面**
- 后台新增"住宿生验证码"管理区域 (templates/admin.html:284-346)
- 生成验证码表单
  - 生成数量：1-20
  - 金币奖励：1-500（默认75）
- 未使用验证码列表（最近20个）
...
**技术实现**
- 新增数据库表：`boarder_codes` (app.py:706-719)
  - code：验证码（唯一索引）
  - coins_reward：金币奖励
  - is_used：是否已使用
  - used_by：使用者ID
...
- 新增用户字段：`boarder_until` (app.py:726)
  - 住宿生状态有效期（TIMESTAMP）

- 核心函数
  - `generate_boarder_code()` (app.py:917-922)：生成8位随机验证码
  - `is_boarder_active()` (app.py:902-914)：检查住宿生状态是否有效
  - `calculate_pet_decay()` 更新 (app.py:842-883)：支持住宿生参数，工作日减半衰减

- 新增路由
  - `admin_generate_boarder_code()` (app.py:1621-1662)：管理员生成验证码
  - `redeem_boarder_code()` (app.py:1665-1750)：住宿生兑换验证码

**衰减计算逻辑**
- 按天逐日计算衰减（处理工作日/周末差异）
- 使用 Python `datetime.weekday()`：0=周一, 4=周五, 5=周六, 6=周日
- 住宿生工作日（周一至周五）：衰减减半
- 住宿生周末 + 走读生全天：正常衰减

**游戏平衡**
- 默认奖励75金币（5天 × 15金币/天 = 75）
- 5天有效期对应一周在校时间（周一到周五）
- 周末回家后正常衰减，鼓励周末互动
